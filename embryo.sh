...
##This code replicates the original TOBIAS paper by Bentsen et al, 2020. The code for merging, sorting, removing PCR duplicates, removing mitochondrial reads, removing blacklisted regions producing BAM files is in the "oct4.sh." script. The blacklist regions are in "hg38-blacklist.v2.bed". It runs MACS2 on merged and sorted BAM files, creates a merged peaks file, runs TOBIAS ATACorrect to correct for Tn5 bias, runs TOBIAS FootprintScores to calculate footprinting scores, and runs TOBIAS BINDetect to identify bound/unbound status of single TF binding sites. The minor modification made to TOBIAS to identify all bound TFs within a chromatin peak is provided in line 25-27 (example shown for chromatin peak #X) and also in the "hipsci.sh" script.
